...
function ItemDetail() {
      ...
      .then(
        (result, reject) => {
          ...
          if (result.size === 0) {
            ...
            let lstProductos = lstProducFilter.map((p) => ({
              ...p.data(),
              id: p.id,
            }));
            ...
            if (productoActual) {
              if (lstProductos[0].cantstk < 1) {
                return M.toast({
                  html: "Actualmente no tenemos disponible en stock.",
                  classes: "rounded",
                });
              }
            } else {
              return M.toast({
                html: "Producto seleccionado no es válido",
  ...
  function btmSumarCont(cantidad) {
    if (
      cantidad <= productoActual.cantstk &&
      cantidad >= productoActual.mincompra
    ) {
  ...
  function BtnCompra({ texto }) {
    ...
    function fnBotonOnclik() {
      ...
      return M.toast({
        html: `Compra Agregada (${cantBoton})`,
    ...
    return (
      <button
        ...
        onClick={fnBotonOnclik}
        disabled={!(!compro && productoActual.cantstk > 0 && cantBoton>0)}
            ...
            <div className="col s12 m7">
              ...
              <div className="card horizontal">
                <div className="card-image">
                  <img
                    src={"../../assets/images/" + productoActual.productimg}
                    alt=""
                    style={{ resize: "inline" }}
                  />
                </div>
                <div className="card-stacked">
                  ...
                  <div className="card-action">
                    <ItemCount
                      fnSumaBoton={btmSumarCont}
                      min={productoActual.mincompra}
                      max={productoActual.cantstk}
                      disable={compro}
                    />
